...
useEffect(()=>{
  ...
  const handleAddCategory = async ()=>{
    if(categoryName){
      const categoryDetails = {categoryName,allVideoes:[]}
...
<div className="container-fluid mt-3">
  {
    ...
    allCategories?.map(categoryDetails=>(
      <div droppable="true" onDragOver={dragOverCategory} onDrop={e=>videoDropOverCategory(e,categoryDetails?.id)} 
      key={categoryDetails?.id} className="border rounded p-3 mb-2">
...
{
categoryDetails?.allVideoes?.length>0 &&
categoryDetails?.allVideoes?.map(video=>(
